...
class SplittedHome extends StatefulWidget {
  final String text;
  final TextStyle textStyle;
  const SplittedHome({Key key, this.text, this.textStyle}) : super(key: key);

  @override
  _SplittedHomeState createState() => _SplittedHomeState();
}

class _SplittedHomeState extends State<SplittedHome> {
  ...
  getSizeAndPosition() {
    RenderBox _cardBox = _pageKey.currentContext.findRenderObject();
    _pageSize = _cardBox.size;
    print(_pageSize);
    setState(() {
      _splitedText();
    });
      ...
      child: Row(
        ...
        children: [
          ...
          IconButton(
            ...
            onPressed: () {
              _pageController.animateToPage(_pageTexts.length,
                  duration: kDuration, curve: kCurve);
            },
          ),
        ],
